...
<a href="<?= Url::to(['/news/' . $post->slug]) ?>" class="news-post card rounded-0 border-gray-600 mb-2 transition text-dark overflow-hidden">
    <div class="news-img-zoom">
        <img src="<?= file_exists(Yii::getAlias('@frontend') . '/web' . $cachedImage) ? $cachedImage : $image->getUrl('500x500') ?>" alt="<?= $image->alt ?: $name ?>" class="card-img img-fluid transition rounded-0" width="500" height="500">
    </div>
