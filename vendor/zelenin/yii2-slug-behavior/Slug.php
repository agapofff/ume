    ...
    protected function getValue($event)
    {
        ...
        if (!empty($owner->{$this->slugAttribute}) && !$this->slugIsEmpty && $this->immutable) {
            ...
            if ($this->attribute !== null) {
                ...
                // если данные для формирования алиаса являются JSON-объектом, берём только значение первого элемента оттуда
                if (!empty($slugParts)){
                    foreach ($slugParts as $key => $slugPart){
                        if (is_string($slugPart) && is_array(json_decode($slugPart, true))){
                            $arr = array_values(json_decode($slugPart, true));
                            $slugParts[$key] = isset($arr[1]) ? $arr[1] : $arr[0];
                        }
                    }
                }                
