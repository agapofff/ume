...
class OneClick extends \yii\base\Widget
{
    ...
    public function init()
    {
        if(is_null($this->content)) {
            $this->content = Yii::t('back', 'Fast order');
        }
    ...
    public function run()
    {
        ...
        if($this->mode == 'popup') {
            ...
            return Html::a($this->content, '#fastOrder'.$this->model->id, ['title' => Yii::t('back', 'Fast order'), 'data-toggle' => 'modal', 'data-target' => '#fastOrder'.$this->model->id, 'class' => 'btn btn-success']);
        } else {
