...
$this->title = $month.' - '.Yii::t('back', 'Order statistics per month');

$this->params['breadcrumbs'][] = ['label' => Yii::t('back', 'Orders'), 'url' => ['/order/default/index']];
$this->params['breadcrumbs'][] = ['label' => Yii::t('back', 'Order statistics'), 'url' => ['index']];
...
<div class="order-stat">
    ...
    <div class="container">
        ...
        ?>
        <p>
            <a href="<?=Url::toRoute(['/order/stat/month/', 'y' => date('Y', $prevMonth), 'm' => date('m', $prevMonth)]);?>">&larr; <?=Yii::t('back', 'Previous');?></a>
            |
            <?php if(date('Ymd', $nextMonth) < date('Ymd')) { ?>
                <a href="<?=Url::toRoute(['/order/stat/month/', 'y' => date('Y', $nextMonth), 'm' => date('m', $nextMonth)]);?>"> <?=Yii::t('back', 'Next');?> &rarr;</a>
            <?php } ?>
            
        </p>
        <table class="table table-hover table-responsive">
            <tr>
                <th><?=Yii::t('back', 'Day');?></th>
                <th><?=Yii::t('back', 'Turnover');?></th>
                <th><?=Yii::t('back', 'Orders count');?></th>
                <th><?=Yii::t('back', 'Average check');?></th>
            </tr>
            <?php $prevStat = false; ?>
                ...
                <tr>
                    <td class="month">
                        <a href="<?=Url::toRoute(['/order/order/index', 'date_start' => "$y-$m-$fd"]);?>">
                            <strong><?=$d;?></strong>
                            <?=Yii::t('back', 'dayname_'.date("w", strtotime("$y-$m-$fd")));?>
                        </a>
                    </td>
                    <?php
