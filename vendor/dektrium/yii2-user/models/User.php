...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'registration_ip'   => Yii::t('front', 'IP'),
            'unconfirmed_email' => Yii::t('front', 'Новый e-mail'),
    ...
    public function attemptConfirmation($code)
    {
        ...
        if ($token instanceof Token && !$token->isExpired) {
            ...
            if (($success = $this->confirm())) {
                ...
                $message = Yii::t('front', 'Поздравляем! Регистрация завершена!');
            } else {
                $message = Yii::t('front', 'Во время подтверждения Вашего аккаунта что-то пошло не так. Пожалуйста, попробуйте ещё раз чуть позже.');
            }
        } else {
            $success = false;
            $message = Yii::t('front', 'Ссылка для подтверждения учетной записи некорректна или устарела. Пожалуйста, запросите новую ссылку для подтверждения Вашей учетной записи.');
        }
    ...
    public function attemptEmailChange($code)
    {
        ...
        if (empty($this->unconfirmed_email) || $token === null || $token->isExpired) {
            \Yii::$app->session->setFlash('danger', Yii::t('front', 'Ваш токен подтверждения недействителен или истек'));
        } else {
            $token->delete();

            if (empty($this->unconfirmed_email)) {
                \Yii::$app->session->setFlash('danger', Yii::t('front', 'Что-то пошло не так'));
                    ...
                    switch ($token->type) {
                        case Token::TYPE_CONFIRM_NEW_EMAIL:
                            ...
                            \Yii::$app->session->setFlash(
                                ...
                                \Yii::t(
                                    'front',
                                    'Ура, почти закончили! Теперь Вам необходимо перейти по ссылке в подтверждающем письме, отправленном на Ваш старый e-mail адрес.'
                                )
                            );
                            break;
                        case Token::TYPE_CONFIRM_OLD_EMAIL:
                            ...
                            \Yii::$app->session->setFlash(
                                ...
                                \Yii::t(
                                    'front',
                                    'Ура, почти закончили! Теперь Вам необходимо перейти по ссылке в подтверждающем письме, отправленном на Ваш новый e-mail адрес.'
                                )
                            );
                            break;
                    }
